...
\item[pql] is the results from fitting the model with \command{glmmPQL} from the \pkg{MASS} package.
\item[startingValues] is a list of starting values for the parameters and random effects, which is obtained from the \command{glmmPQL} result.
\end{description}  

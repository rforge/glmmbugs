...
As the results stored in \var{bacResult} are based on the reparametrized model, we use \command{restoreParams} is used to restore the original parametrization from (\ref{eq:model}). 
...
\subsection{Pre and Post-WinBUGS commands}
Before running WinBUGS, \command{glmmBUGS} is needed to generate a list of data for running the MCMC simulation. In order to run \command{glmmBUGS}, the data needed to be in the format for running the generalized linear model. After running WinBUGS, a series of commands in the \pkg{glmmBUGS} can be used to check the simulation results. \command{resoreParams} is used to give the original names to random effect levels.  \command{summaryChain} is used to compute summary statistics for each parameter. \command{summaryChain} is used to make time series plots of the parameters (not the random effects) of an MCMC run.

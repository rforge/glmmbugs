...
myLgcp=simLgcp(mymodel, torontoNightR, betas=c(layer=0.025), 
...
<<inlaResPlot,fig.cap="Intensity surfaces",fig.subcap=c("true","posterior mean"),out.width="0.45\\textwidth">>=
map.new(torontoBorderR,legendRight=TRUE)
plot(myLgcp$raster[['intensity']],add=TRUE)
points(myLgcp$events, col='#00000050', cex=0.2)


map.new(torontoBorderR,legendRight=TRUE)
plot(fit$raster[['predict.exp']],add=TRUE)
...
# range
plot(fit$parameters$range$post,col='red', type='l', xlim=c(0, 3*10^5))
...
# sd
plot(fit$parameters$sd$post,col='red',type='l', xlim=c(0,50))

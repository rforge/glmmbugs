...
forBugs = glmmBUGS(counts +offset ~ night,
		...
		priors=c(phicell="dunif(5000,40000)", 
				intercept="dunif(-20, -10)",
				SDcell = "dunif(0.1, 4)",
...
getInits = function() { 
	list(
		intercept = forBugs$startingValues$intercept + runif(1,-1,1),
		betacell =runif(1, -0.05, 0.05),
		SDcell = runif(1, 0.9, 1.1),
		phicell = runif(1, 19000, 21000),
...
bugsFit = bugs(forBugs$ragged, inits=getInits,
		  ...
		  n.iter = 60, n.burnin = 10,
		  parameters = c(names(getInits())),
		  n.thin = 5) 
d2=date()
...
checkChain(theParams, c("rangecell","SDcell","intercept"), one=FALSE)
@
...
<<condSimuLoa,echo=TRUE,cache=TRUE,eval=FALSE>>=
theParams$raster = 
		grfConditional(
		data=data,
		y =theParams$RvillageID,
		param=list(
...
<<gridCmeanSim,echo=TRUE,eval=FALSE>>=
simGridPmean = overlay(theParams$raster, fun=mean)
simGridPmean
@


<<bugsResPlot,fig.cap="Surfaces from Bugs",fig.subcap=c("Rand eff true", "Rand eff post mean", "One post'r sample","another sample"),out.width="0.45\\textwidth">>=
mycol = rev(heat.colors(100))
map.new(torontoBorderR,legendRight=TRUE)
plot(torontoTilesR,add=TRUE)
...
<<bugsPostr,fig.cap='parameter distributions from Openbugs',fig.subcap=c('range','sd','night'),out.width='0.3\\textwidth',fig.height=4,fig.width=5,echo=FALSE>>=


# range
plot(density(simParams$rangecell),main='',col='red', 
		xlab='range', ylab='density')
abline(v=myLgcp$par$random["range"])

# sd
plot(density(simParams$SDcell),main='',col='red', 
		xlab='range', ylab='density')
abline(v=sqrt(myLgcp$par$random["variance"]))

# coefficient
plot(density(simParams$betacell),main='',col='red', 
		xlab='range', ylab='density')
abline(v=myLgcp$par$fixed["layer"])
@
